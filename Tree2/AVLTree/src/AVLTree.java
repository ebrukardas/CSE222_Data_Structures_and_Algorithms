...
public class AVLTree<E extends Comparable<E>> extends BinarySearchTreeWithRotate<E> {
	...
	private void addTree(Node<E> node){
		if (node == null) 
			return;
		else {
			add(node.data);
	...
	private void preOrderTraverse(Node < E > node, int depth, StringBuilder sb) {
		...
		if (node == null) 
			...
			sb.append(node.toString() + " h:" + heightOfTree(node) + " b:" + balancedTree(node));
			sb.append("\n");
	...
	public boolean balancedTree(Node<E> node){
		if(node==null)
			return true;
		else{
			int leftHeight = heightOfTree(node.left);
			int diff = Math.abs( leftHeight - heightOfTree(node.right) );
			return balancedTree(node.left) && balancedTree(node.right) && diff<=1;
		}
	}
...
//System.out.println(localRoot.balance + "&"+ AVLNode.RIGHT_HEAVY);
	...
	private AVLNode<E> rebalanceRight(AVLNode<E> localRoot) {
		...
		if (rightChild.balance < AVLNode.BALANCED) {
			...
			// and the root (new leftt child) will both be balanced
			// after the rotation.
			rightChild.balance = AVLNode.BALANCED;
			localRoot.balance = AVLNode.BALANCED;
		}
